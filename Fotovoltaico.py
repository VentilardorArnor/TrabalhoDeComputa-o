import pygame
import pygame.freetype  # --- MODIFICAÇÃO: Usaremos freetype para renderizar texto
...
# --- MODIFICAÇÃO: Dicionário de specs agora inclui a potência (wattage) ---
PAINEL_SPECS = {
...
out vec3 FragPos;out vec3 Normal;out vec2 TexCoords;out vec4 FragPosLightSpace;
uniform mat4 projection;uniform mat4 view;uniform mat4 model;uniform mat4 lightSpaceMatrix;
void main(){FragPos=vec3(model*vec4(aPos,1.0));Normal=mat3(transpose(inverse(model)))*aNormal;TexCoords=aTexCoords;
FragPosLightSpace=lightSpaceMatrix*vec4(FragPos,1.0);gl_Position=projection*view*model*vec4(aPos,1.0);}
"""
FRAGMENT_SHADER_SHADOW="""
#version 330 core
out vec4 FragColor;in vec3 FragPos;in vec3 Normal;in vec2 TexCoords;in vec4 FragPosLightSpace;
uniform sampler2D diffuseTexture;uniform sampler2D shadowMap;uniform vec3 lightPos;uniform vec3 viewPos;
float calculateShadow(vec4 fragPosLightSpace,vec3 normal,vec3 lightDir){
vec3 projCoords=fragPosLightSpace.xyz/fragPosLightSpace.w;projCoords=projCoords*0.5+0.5;
if(projCoords.z>1.0)return 0.0;float closestDepth=texture(shadowMap,projCoords.xy).r;float currentDepth=projCoords.z;
float bias=max(0.05*(1.0-dot(normal,lightDir)),0.005);float shadow=currentDepth-bias>closestDepth?1.0:0.0;return shadow;}
void main(){vec3 color=texture(diffuseTexture,TexCoords).rgb;vec3 normal=normalize(Normal);vec3 lightColor=vec3(1.0);
vec3 ambient=0.25*color;vec3 lightDir=normalize(lightPos-FragPos);float diff=max(dot(normal,lightDir),0.0);
vec3 diffuse=diff*lightColor;float shadow=calculateShadow(FragPosLightSpace,normal,lightDir);
vec3 lighting=(ambient+(1.0-shadow)*diffuse)*color;FragColor=vec4(lighting,1.0);}
"""

# --- Classe da Câmera (sem alterações) ---
class Camera:
    def __init__(self,position=np.array([0.,2.5,8.]),up=np.array([0.,1.,0.]),yaw=-90.,pitch=0.):
        self.position=position;self.world_up=up;self.yaw=yaw;self.pitch=pitch;self.movement_speed=5.;self.mouse_sensitivity=0.1
...
# --- Dados da Geometria (sem alterações) ---
...
# --- MODIFICAÇÃO: Nova função para recalcular as posições dos painéis ---
def update_panel_positions(rows, cols):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            z = (i - (rows - 1) / 2.0) * spacing_z - 10.0 # Move o conjunto um pouco para trás
            positions.append(np.array([x, 0, z]))
    return positions

def draw_scene(shader,vao_dict,tex_dict,panel_positions,panel_specs,panel_tilt):
    ...
    for position in panel_positions:
        ...
        angle_rad=np.radians(panel_tilt)
        panel_rot=np.array([[1,0,0,0],[0,np.cos(angle_rad),-np.sin(angle_rad),0],[0,np.sin(angle_rad),np.cos(angle_rad),0],[0,0,0,1]],dtype=np.float32)
        panel_scale=np.diag(list(panel_size)+[1.]).astype(np.float32)
        model=panel_scale@panel_rot@panel_trans@base_trans
        glUniformMatrix4fv(glGetUniformLocation(shader,"model"),1,GL_FALSE,model)
        glDrawArrays(GL_TRIANGLES,0,36)

# --- MODIFICAÇÃO: Nova função para renderizar o HUD ---
...
def main():
    ...
    # --- MODIFICAÇÃO: Inicialização de fontes e variáveis de estado ---
    hud_font = pygame.freetype.SysFont("Arial", 24)
    hora_atual = 12.0
    tipo_painel_selecionado = 610
    num_rows, num_cols = 2, 3
    panel_tilt_angle = 20.0 # Inclinação dos painéis em graus
    ...
    textures={'grass':load_texture("grass.png"),'panel':load_texture("solar_cell.png"),'metal':load_texture("metal_frame.png")}
    if -1 in textures.values():pygame.quit();sys.exit()
    vaos={'cube':setup_geometry()[0],'plane':setup_geometry()[1]}
    panel_positions = update_panel_positions(num_rows, num_cols)
    
    # Configuração do FBO
    SHADOW_WIDTH,SHADOW_HEIGHT=2048,2048
    depthMapFBO=glGenFramebuffers(1);glBindFramebuffer(GL_FRAMEBUFFER,depthMapFBO)
    depthMap=glGenTextures(1);glBindTexture(GL_TEXTURE_2D,depthMap)
    ...
    while True:
        ...
        # --- MODIFICAÇÃO: Novos handlers de evento para + e - ---
        for event in pygame.event.get():
            ...
            if event.type==pygame.KEYDOWN:
                ...
                if event.key==pygame.K_EQUALS or event.key==pygame.K_PLUS:
                    num_cols=min(10,num_cols+1)
                    panel_positions=update_panel_positions(num_rows,num_cols)
                if event.key==pygame.K_MINUS:
                    num_cols=max(1,num_cols-1)
                    panel_positions=update_panel_positions(num_rows,num_cols)
        ...
        # --- MODIFICAÇÃO: Cálculo da Geração de Energia ---
        panel_normal_angle_rad = np.radians(panel_tilt_angle)
        panel_normal = np.array([0, np.cos(panel_normal_angle_rad), np.sin(panel_normal_angle_rad)])
        light_dir = lightPos - np.array([0, 2, 0]) # Posição média dos painéis
        ...
        # PASSO 1: RENDERIZAÇÃO DE PROFUNDIDADE
        lightProjection=ortho(-40.,40.,-40.,40.,1.,80.)
        lightView=camera._look_at(lightPos,np.array([0.,0.,0.]),np.array([0.,1.,0.]))
        ...
        draw_scene(shaders['depth'],vaos,textures,panel_positions,specs_do_painel,panel_tilt_angle)
        glBindFramebuffer(GL_FRAMEBUFFER,0)

        # PASSO 2: RENDERIZAÇÃO NORMAL
        glViewport(0,0,screen_width,screen_height)
        glClearColor(0.5,0.8,1.0,1.0);glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
        glUseProgram(shaders['shadow'])
        projection=perspective(45.0,screen_width/screen_height,0.1,100.0)
        ...
        draw_scene(shaders['shadow'],vaos,textures,panel_positions,specs_do_painel,panel_tilt_angle)

        # --- MODIFICAÇÃO: Renderiza o HUD por cima de tudo ---
        minutos = int((hora_atual % 1) * 60)
        titulo = f"Fazenda | Hora: {int(hora_atual):02d}:{minutos:02d} | Painel: {tipo_painel_selecionado}W"
        pygame.display.set_caption(titulo)
        
        hud_text = f"Geração Total: {total_power_watts / 1000.0:.2f} kW"
