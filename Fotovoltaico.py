...
# --- INÍCIO: NOVA FUNÇÃO PARA GERAR UM CILINDRO ---
def generate_cylinder_vertices(radius=0.5, height=1.0, sides=16):
    vertices = []
    # Paredes Laterais
    for i in range(sides):
        angle_i = (i / sides) * 2 * math.pi
        angle_j = ((i + 1) / sides) * 2 * math.pi
        x_i, z_i = radius * math.cos(angle_i), radius * math.sin(angle_i)
        x_j, z_j = radius * math.cos(angle_j), radius * math.sin(angle_j)
        # Triângulo 1
...
# --- FIM: NOVA FUNÇÃO PARA GERAR UM CILINDRO ---
...
# --- INÍCIO: MODIFICAÇÃO EM SETUP_GEOMETRY ---
def setup_geometry():
    # Setup para o Cubo (sem alteração)
    ...
    # Setup para o Chão (sem alteração)
    ...
    return cubeVAO, planeVAO, cylinderVAO # Retorna o novo VAO do cilindro
# --- FIM: MODIFICAÇÃO EM SETUP_GEOMETRY ---
...
def draw_scene(shader, vao_dict, tex_dict, panel_positions, panel_specs, panel_tilt, panel_azimuth, battery_percentage):
    ...
    model = np.identity(4, dtype=np.float32); glUniformMatrix4fv(glGetUniformLocation(shader, "model"), 1, GL_FALSE, model); glDrawArrays(GL_TRIANGLES, 0, 6)
    panel_size = panel_specs['size']
    for position in panel_positions:
        post_height = 2.
        
        # --- INÍCIO: MODIFICAÇÃO PARA DESENHAR O SUPORTE CILÍNDRICO ---
        # Em vez de vao_dict['cube'], usamos vao_dict['cylinder']
        glBindVertexArray(vao_dict['cylinder'])
        # --- FIM: MODIFICAÇÃO PARA DESENHAR O SUPORTE CILÍNDRICO ---
        ...
        # O número de vértices de um cilindro é diferente de um cubo
        glDrawArrays(GL_TRIANGLES, 0, 16 * 12) # 16 lados * (2 triângulos para a parede + 2 triângulos para as tampas) * 3 vértices/triângulo = 16*12
        
        glBindVertexArray(vao_dict['cube']) # Volta a usar o cubo para o painel
        ...
        glUniformMatrix4fv(glGetUniformLocation(shader, "model"), 1, GL_FALSE, model); glDrawArrays(GL_TRIANGLES, 0, 36)
    ...
    scale = np.diag([1.5, 2.0, 1.0, 1.0]).astype(np.float32)
    model = scale @ base_trans
    glUniformMatrix4fv(glGetUniformLocation(shader, "model"), 1, GL_FALSE, model); glDrawArrays(GL_TRIANGLES, 0, 36)
    
    BAR_MAX_HEIGHT = 1.8; BAR_WIDTH = 0.4
    glBindTexture(GL_TEXTURE_2D, tex_dict['red'])
    bar_pos = battery_pos + np.array([0, 0, 0.51])
    ...
    glBindVertexArray(vao_dict['cube']); sign_pos = np.array([0, 0, 2.0])
    glBindTexture(GL_TEXTURE_2D, tex_dict['metal'])
...
def main():
    ...
    glBindTexture(GL_TEXTURE_2D, sign_texture); glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST)
    ...
    # --- INÍCIO: MODIFICAÇÃO PARA RECEBER O VAO DO CILINDRO ---
    cube_vao, plane_vao, cylinder_vao = setup_geometry()
    vaos = {'cube': cube_vao, 'plane': plane_vao, 'cylinder': cylinder_vao}
    # --- FIM: MODIFICAÇÃO PARA RECEBER O VAO DO CILINDRO ---
    ...
    while True:
        ...
        total_power_kw = (len(panel_positions)*specs_do_painel['wattage']*efficiency) / 1000.0
